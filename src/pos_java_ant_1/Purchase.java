...
public class Purchase extends javax.swing.JPanel {
    ...
    public void data_load(){
        ...
        //product
        String sup = customer_combo.getSelectedItem().toString();
        try {
            Statement s = db.myCon().createStatement();
            ResultSet rs = s.executeQuery("SELECT * FROM product WHERE sup_name = '"+sup+"'");
        ...
        if (row != 0) {
            ...
            for (int i = 0; i < row_count; i++) {
                ...
                String tot_price = tm.getValueAt(i, 5).toString();
                   ...
                   Statement s = db.myCon().createStatement();
                    //cart table
                    s.executeUpdate("INSERT INTO pur_cart (pid, prod_name, bar, qty, unit_price, total_price) VALUES ('"+pid+"','"+name+"','"+bar+"','"+qty+"','"+u_price+"','"+tot_price+"')"); 
            }
            ...
            try {
                ...
                Statement s = db.myCon().createStatement();
                //sales table
                s.executeUpdate("INSERT INTO purchase (pid, sid, s_name, t_qty, bill, status, bal, method) VALUES ('"+iid+"','"+customer_ID+"','"+name+"','"+t_qty+"','"+bill+"','"+status+"','"+bal+"','"+method+"')");
            } catch (SQLException e) {
                try {
                Statement st = db.myCon().createStatement();
                st.executeUpdate("CREATE TABLE `purchase` (\n" +
...
")");
            ...
            try {
                ...
                Statement s = db.myCon().createStatement();
                s.executeUpdate("UPDATE p_extra SET val = '"+current_id+"' WHERE xid = 1");
            } catch (SQLException e) {
                System.out.println(e);
                try {
                Statement st = db.myCon().createStatement();
                st.executeUpdate("CREATE TABLE `p_extra` (\n" +
"  `xid` INT NOT NULL AUTO_INCREMENT,\n" +
"  `val` VARCHAR(10) NOT NULL\n" +
")");
                st.executeUpdate("INSERT INTO p_extra (val) VALUES ('"+inid.getText()+"')");
                JOptionPane.showMessageDialog(null, "Please try again in first attemt when application launch");
            } catch (Exception ex) {
                System.out.println(ex);
            }
            ...
            try {
                Statement st = db.myCon().createStatement();
                st.executeUpdate("CREATE TABLE `pur_cart` (\n" +
